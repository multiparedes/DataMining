...
As we commented before the columns of X, that acts as an identifier can be removed from the df as it doesn't give us any relevant information, the next step is to encode the column sex as a binary, where 0 represents a female penguin and 1 a male. The column year we are going to apply a One-Hot Encoding, were we have 3 columns, year_2007, year_2008 and year_2009 and we will mark the observed value with a 1 and the rest with a 0. To finish the transformation on our df we are going to transform the variable body_mass_g into body_mass_kg, this way we are going to have smaller values maintaining the same relation.

```{r}
df$sex <- ifelse(df$sex == "female", 0, 1)

df$year_2007 <- ifelse(df$year == '2007', 1, 0)
df$year_2008 <- ifelse(df$year == '2008', 1, 0)
df$year_2009 <- ifelse(df$year == '2009', 1, 0)
...
categorical_data <- df[, c("sex", "year_2007", "year_2008", "year_2009")]

scaled_numeric_data <- scale(numeric_data)

df_scaled <- cbind(scaled_numeric_data, categorical_data)
```
...
```{r}
df_f <- cbind(as.data.frame(pca_result$x[,1:2]), categorical_data)
head(df_f)
```
...
plot(pca_result$x[, 1:2], col = groups)
text(x = pca_result$x[, 1:2], labels=df$body_mass_kg, pos = 3, cex = 0.5, col = groups)
